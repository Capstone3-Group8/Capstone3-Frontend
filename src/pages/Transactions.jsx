...
export default function TransactionsPage() {
  ...
  /*
   * Determine whether an account is a liability.
   * Credit cards, loans, debt, and mortgages are treated as liabilities.
   */
  ...
  function getCurrentAccountBalance(account) {
    ...
    return transactions.reduce((balance, transaction) => {
      if (
        Number(transaction.account_id) !== Number(account.id)
      ) {
        return balance;
      }
  ...
  async function handleAutoCategorize() {
    ...
    try {
      ...
      setError(
        err.message || "Could not categorize transactions"
      );
    } finally {
    ...
    <section className="mx-auto w-full max-w-5xl p-4">
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-2 text-xl font-bold !text-(--text-h)">
                    $
                    {getCurrentAccountBalance(account).toFixed(
                      2
                    )}
                  </p>
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <button
          ...
          disabled={
            isCategorizing || transactions.length === 0
          }
      ...
      {transactions.length === 0 ? (
        ...
        <div className="rounded-xl border border-(--border) bg-(--panel) p-8 text-center">
          ...
          <p className="mt-1 text-sm text-(--text)">
            Try a different category, account, amount, type, or
            note.
          </p>
        </div>
      ) : (
