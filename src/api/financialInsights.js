...
export async function suggestCategoryForTransaction(
  ...
  const response = await fetch(
    `${BASE_URL}/api/financial-insights/suggest-category`,
    {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ transaction, existingCategories }),
    },
  );
  ...
  if (!response.ok) {
    const body = await response.json().catch(() => ({}));
    throw new Error(
      body.error || `Could not suggest a category (${response.status})`,
    );
  }
